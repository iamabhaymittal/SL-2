...
int main()
{
	int i,np,nc;  // np is no. of producers and nc is no. of consumers
	pthread_t pid[buff_size];
	pthread_t cid[buff_size];
	int c[]={1,2,3,4,5};
	...
	for(i=0;i<np;i++) //creating threads for producer
	{
		int *arg=&i;
	...
	for(i=0;i<nc;i++) //creating threads for consumer
	{
		int *arg=&i;
	...
	for(i=0;i<np;i++) //joining producer thread
	{
		pthread_join(pid[i],NULL);
		printf("\n-------- Thread for Producer %d Joined at End.\n",i);
	}
	for(i=0;i<nc;i++) //joining consumer thread
	{
...
void init()
{
	...
	pthread_mutex_init(&b1.lock,NULL); //locks the region of code
